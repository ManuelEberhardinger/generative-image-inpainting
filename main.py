...
class Run(object):
    def __init__(self, args):
        # Data loader
        self.data_loader = get_loader(args.DATASET, args.IMAGE_PATH, args.METADATA_PATH,
    ...
    def init_network(self, args):
        ...
        if torch.cuda.is_available():
            ...
            self.torch_L1 = nn.L1Loss().to(self.device)
        ...
        for epoch in range(start, self.num_epochs):
            for batch, real_image in enumerate(self.data_loader):  # real_image : B x 3 x H x W
                ...
                if batch % self.print_every == 0:
                    ...
                    print('=====================================================')
                    print('reconstruction loss: ', self.loss['recon'].data.item())
                    print('g loss: ', self.loss['g_loss'].data[0])
                    print('d loss: ', self.loss['d_loss'].data.item())

                    if self.show_images and not self.device == 'cuda':
